...
def read_issue(repo: str, node) -> Issue:
    ...
    labels = _labels(node)
    issue = Issue(
        repo, node["number"], node["title"], created_at, updated_at, closed_at, labels
    )
    return issue


def _labels(node):
    """Retrieve all labels from a node"""
...
def parse_date(iso_timestamp: str) -> datetime:
    return (
        ...
        else datetime.datetime.strptime(iso_timestamp, "%Y-%m-%dT%H:%M:%S%z").replace(
            tzinfo=None
        )
    )
